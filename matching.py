...
def read_targets_from_csv(file_path):
    ...
    rows = []
    with open(file_path, newline='', encoding=config['ENCODING']['input_encoding']) as csvfile:
    ...
    # Process CSV data
    for row in rows:
        id_values = [list(row.values())[int(config['IDS'][f'id{i}']) - 1] for i in range(1, len(config['IDS']) + 1) if config['IDS'][f'id{i}']]
        ...
        choices = [list(row.values())[int(config['CHOICES'][f'choice{i}']) - 1] for i in range(1, len(config['CHOICES']) + 1) if config['CHOICES'][f'choice{i}']]
        ...
        for preference in target_preferences[target]:
            if len(matches[preference]) < recruiting_numbers[preference]:
                ...
                for index, matched_target in enumerate(matches[preference]):
                    score = target_params_dict[matched_target][preference + "SCORE"]
                ...
                if (target_params_dict[target][preference + "SCORE"] > min_score) or \
                   (target_params_dict[target][preference + "SCORE"] == min_score and 
                    int(target_params_dict[target]['hash'], 16) > int(min_hash, 16)):
...
def merge_dicts_with_final_result(target_org, target_params, final_result, final_result2):
    ...
    """
    merged = {}
    reversed_final_result = {id: role for role, ids in final_result.items() for id in ids}
    reversed_final_result2 = {id: role for role, ids in final_result2.items() for id in ids}
...
def merged_data_to_csv(merged, file_path):
    ...
    with open(file_path, 'w', newline='', encoding=config['ENCODING']['output_encoding']) as csvfile:
...
logging.basicConfig(filename='matching.log', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
...
recruiting_numbers = {key: int(value) for key, value in config['RECRUIT'].items()}
...
target_org, target_params, target_preferences = read_targets_from_csv(input_file)
...
elected_result = stable_matching(target_preferences, target_params_dict, recruiting_numbers)
updated_target_preferences = remove_matched_targets_from_preferences(elected_result, target_preferences)
alternate_result = stable_matching(updated_target_preferences, target_params_dict, reserve_numbers)
...
merged_data = merge_dicts_with_final_result(target_org, target_params_dict, elected_result, alternate_result)
